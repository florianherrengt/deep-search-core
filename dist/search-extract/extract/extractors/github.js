...
export function isGithubRepoOverviewUrl(url) {
    ...
    if (RESERVED_FIRST_SEGMENTS.has(owner.toLowerCase()))
        return false;
    if (repo.toLowerCase() === "settings")
        return false;
...
export function isGithubNotFoundHtml(html) {
    const $ = load(html);
...
function extractCounters($) {
    const counters = [];
    const starsText = firstNonEmpty($("#repo-stars-counter-star").text(), $("a[href$='/stargazers']").text(), $("a[href$='/stargazers/']").text());
    if (starsText)
        counters.push({ label: "Stars", value: parseCountInt(starsText) ?? starsText });
    const forksText = firstNonEmpty($("#repo-network-counter").text(), $("a[href$='/forks']").text(), $("a[href$='/network/members']").text());
    if (forksText)
        counters.push({ label: "Forks", value: parseCountInt(forksText) ?? forksText });
    const watchersText = firstNonEmpty($("#repo-notifications-counter").text(), $("a[href$='/watchers']").text());
    if (watchersText)
...
function extractHomepage($) {
    let homepage = null;
    $(".BorderGrid-row a[href^='http'], .BorderGrid-cell a[href^='http'], a[data-octo-dimensions*='homepage']").each((_, el) => {
        if (homepage)
            return;
...
function extractLicense($) {
    let license = null;
    $("a[href*='/blob/'][href*='LICENSE' i], a[href*='/blob/'][href*='COPYING' i], a[href*='/blob/'][href*='NOTICE' i]").each((_, el) => {
        if (license)
            return;
        const text = normalizeWhitespace($(el).text());
...
function extractLanguages($) {
    ...
    const languagesHeading = $("h2, h3, summary")
    ...
    if (languagesHeading.length) {
        ...
        container
            ...
            .each((_, el) => {
            const $el = $(el);
            const name = normalizeWhitespace($el.find("span.color-fg-default.text-bold, span[itemprop='name']").first().text() ||
                ($el.is("li") ? $el.find("span").first().text() : ""));
            const percent = normalizeWhitespace($el.find("span:not(.color-fg-default):not([itemprop])").last().text());
...
function extractContributors($) {
    ...
    const topNames = [];
    $("img[src*='/u/'], a[href$='/graphs/contributors'] img, .avatar, a[data-hovercard-type='user'] img, img[class*='avatar']").each((_, el) => {
        if (topNames.length >= 8)
            return;
...
function extractCommitInfo($) {
    let count = null;
    const commitLinks = $("a[href*='/commits/'], a[href$='/commits'], a[aria-label*='commits' i], a[title*='commits' i]");
...
export function parseGithubRepoHtml(html) {
    const $ = load(html);
    if (isGithubNotFoundHtml(html))
        return null;
    const fullName = firstNonEmpty(metaContent($, ["og:title"]), (() => {
...
function formatParsedRepo(repo) {
    ...
    if (repo.contributors.topNames.length) {
        contribParts.push(`Top: ${repo.contributors.topNames.map((n) => `@${n}`).join(", ")}`);
    }
    if (contribParts.length)
        lines.push(contribParts.join(" · "));
    const commitParts = [];
    if (repo.commits.count)
        commitParts.push(`Commits: ${repo.commits.count}`);
    if (repo.commits.lastCommitDate)
        commitParts.push(`Last commit: ${repo.commits.lastCommitDate}`);
    if (commitParts.length)
        lines.push(commitParts.join(" · "));
    const activeFlags = [];
    if (repo.flags.archived)
        activeFlags.push("archived");
    if (repo.flags.fork)
        activeFlags.push("fork");
    if (repo.flags.disabled)
        activeFlags.push("disabled");
    if (activeFlags.length)
        lines.push(`Status: ${activeFlags.join(", ")}`);
    if (repo.readme) {
        lines.push("");
        lines.push("## README");
        lines.push("");
