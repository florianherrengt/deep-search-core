...
export async function downloadYouTubeSubtitles(config) {
    ...
    const track = selectCaptionTrack(tracks, config.languageCode, Boolean(config.preferAutoGenerated));
...
async function fetchCaptionTracks({ videoId, languageCode, fetchImpl, signal, }) {
    ...
    if (!response.ok) {
        throw new Error(`YouTube video page failed with HTTP ${response.status}${response.statusText ? ` ${response.statusText}` : ""}.`);
    }
...
function selectCaptionTrack(tracks, languageCode, preferAutoGenerated) {
    ...
    const candidates = requestedLanguage
        ? tracks.filter((track) => languageMatches(track.languageCode, requestedLanguage))
        : tracks;
    if (candidates.length === 0) {
        throw new Error(`No subtitles found for language "${languageCode}". Available languages: ${formatAvailableLanguages(tracks)}.`);
    }
    if (!requestedLanguage) {
        const englishCandidates = candidates.filter((track) => languageMatches(track.languageCode, "en"));
...
async function fetchCaptionCues(track, fetchImpl, signal) {
    ...
    if (!response.ok) {
        throw new Error(`YouTube subtitles failed with HTTP ${response.status}${response.statusText ? ` ${response.statusText}` : ""}.`);
    }
    const text = await response.text();
    if (!text.trim()) {
        throw new Error("YouTube returned an empty subtitle response. This video may require YouTube's proof-of-origin token.");
    }
    let json;
    try {
...
//# sourceMappingURL=youtube-subtitles.js.map