...
export function createAiSdkSummarizer(model) {
...
export function createAiSdkSearchTool(engine, provider, description) {
    return tool({
        description,
        strict: true,
...
const extractPageContentInputSchema = z.object({
    ...
    method: z
        ...
        .optional()
        .describe("Extraction method. 'auto' tries fetch then falls back to webview. 'fetch' forces HTTP-only. 'webview' forces browser rendering."),
});
...
export function createAiSdkExtractPageContentTool(engine, options) {
